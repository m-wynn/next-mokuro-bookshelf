...
let storageKey = "mokuro_" + decodeURIComponent(window.location.pathname);
...
function updatePage(new_page_idx) {
    ...
    getPage(state.page_idx).classList.remove("preload");
    ...
    if (!state.singlePageView && state.page_idx < num_pages - 1 && !isPageFirstOfPair(state.page_idx + 1)) {
        ...
        getPage(state.page2_idx).classList.remove("preload");
    ...
    // Preload next two pages if available
    for (let i = 1; i < 3; i++) {
        let next_page_id = state.page_idx + i + (state.singlePageView ? 0 : 1);
        if (next_page_id < num_pages) {
            getPage(next_page_id).classList.add("preload");
            getPage(next_page_id).style.display = "inline-block";
        }
    }
